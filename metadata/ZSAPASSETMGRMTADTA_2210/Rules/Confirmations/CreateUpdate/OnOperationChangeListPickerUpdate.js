...
import actPickerQueryOptions from '../../../../SAPAssetManager/Rules/Confirmations/CreateUpdate/ActivityPickerQueryOptions';
import variancePickerQueryOptions from '../../../../SAPAssetManager/Rules/Confirmations/CreateUpdate/VariancePickerQueryOptions';
import SubOperationQueryOptions from '../../../../SAPAssetManager/Rules/Confirmations/CreateUpdate/SubOperationPickerQueryOptions';
import technicianNameQueryOption from '../../TechnicianNameQueryOption';
...
export default function OnOperationChangeListPickerUpdate(pageProxy) {
    ...
    return Promise.all([SubOperationQueryOptions(pageProxy, workorder, operation), actPickerQueryOptions(pageProxy), variancePickerQueryOptions(pageProxy), technicianNameQueryOption(pageProxy)]).then(function(results) {
            ...
            return redrawListControl(pageProxy, 'SubOperationPkr', results[0], count > 0 && pageProxy.binding.IsOnCreate, clearSubOperationValue).then(() => {
                return redrawListControl(pageProxy, 'ActivityTypePkr', results[1], enable).then(() => {
                    ...
                    return redrawListControl(pageProxy, 'VarianceReasonPkr', results[2], enable).then(() => {
                        return redrawListControl(pageProxy, 'TechnicianNamePkr', results[3], enable).then(() => {
                            return true;
                            });
                    });
                });
