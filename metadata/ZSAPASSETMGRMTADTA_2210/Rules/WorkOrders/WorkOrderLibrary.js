...
export class WorkOrderLibrary {
    ...
    */
    ...
    static setPM04CreateUpdateFields(context) {
        try {
                let formCellContainer = context.getControl('FormCellContainer');
                let mmtContact = formCellContainer.getControl('MMTContact');
                let orderType = libCommon.getTargetPathValue(context, '#Page:WorkOrderCreateUpdatePage/#Control:TypeLstPkr/#SelectedValue');
                if (orderType === "PM04"){
                    mmtContact.setVisible(true);
                } else {
                    mmtContact.setVisible(false);
                }
                return Promise.resolve();
        } catch (error) {
            Logger.error(context.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryWorkOrders.global').getValue(),`setServiceOrderCreateUpdateFields(context) Error: ${error}`);
...
export class WorkOrderEventLibrary {
                ...
                case 'PlanningPlantLstPkr':
                    return libWoControls.updateOrderType(context).then(() => {
                        return libWoControls.updateFunctionalLocation(context).then(() => {
                            ...
                            libWoControls.updatePMActType(context);
                            });
                        });
                    });
                case 'TypeLstPkr':
                    return libWoControls.updatePriority(context).then(() => {
                        return libWo.setServiceOrderCreateUpdateFields(context).then(() => {
                            libWoControls.updatePMActType(context);
                            libWo.setPM04CreateUpdateFields(context);
                        });
                    });
            ...
            case 'FuncLocHierarchyExtensionControl':
                {
                    ...
                    let funcLoc, equipmentControlValue;
                    if (controlName === 'FuncLocHierarchyExtensionControl') {
                        funcLoc = context.getControl('FormCellContainer').getControl('FuncLocHierarchyExtensionControl').getValue();
                        if (funcLoc) {
                            result = "$orderby=FuncLocId&$filter=(FuncLocIdIntern eq '" + funcLoc + "')";
                            } else {
                                equipmentControlValue = context.getControl('FormCellContainer').getControl('EquipHierarchyExtensionControl').getValue();
                            if (equipmentControlValue) {
                                extensionPromise = context.read('/SAPAssetManager/Services/AssetManager.service', 'MyEquipments', ['FuncLocIdIntern'], `$filter=EquipId eq '${equipmentControlValue}'&$expand=FunctionalLocation&$orderby=EquipId`).then( results => {
                                    if (results.length > 0 && results.getItem(0).FuncLocIdIntern) {
                                        funcLoc = results.getItem(0).FuncLocIdIntern;
                                        result = "$orderby=FuncLocId&$filter=(FuncLocIdIntern eq '" + funcLoc + "')";
                                    } else {
                                        funcLoc.setData('');
                                        result = "$orderby=FuncLocId";
                                    }
                                    return Promise.resolve(true);
                                });
    ...
    static CreateUpdateOnCommit(pageProxy) {
        ...
        //Temporary Workaround for an issue where the hierarchy list picker is wiping out the binding on the page. MDK issue logged https://jira.tools.sap/browse/MDKBUG-585.
        ...
        if (onCreate) {
            return libWo.isServiceOrderCreateUpdate(pageProxy).then(isServiceOrder => {
                ...
                let pmActType = libWoControls.getPMActivityType(pageProxy);  
                ...
                let woDefaultValue = {
                    ...
                    'MaintenanceActivityType':pmActType
                };
                ...
                // Start of the code changes done for CMW_Enhanc_01 - Current Location
                // pageProxy.executeAction('/ZSAPASSETMGRMTADTA_2210/Actions/UpdateLocationWOCreate.action');
            ...
            return markedJobCreateUpdateOnCommit(pageProxy).then(() => {
                return pageProxy.executeAction('/SAPAssetManager/Actions/WorkOrders/WorkOrderUpdate.action').then(() => {
                    return pageProxy.executeAction('/ZSAPASSETMGRMTADTA_2210/Actions/UpdateLocationWOChange.action')
                });
            });
        }
    }
...
export class WorkOrderControlsLibrary {
    ...
    /**
     * PMActivityType getter
     * @param {IPageProxy} pageProxy
     */
    static getPMActivityType(pageProxy) {
            let PMActivityType = libCommon.getTargetPathValue(pageProxy, '#Page:WorkOrderCreateUpdatePage/#Control:PMActivityTypeLstPkr/#Value');
            return libCommon.getListPickerValue(PMActivityType);
        }
     ...
     * @param {IPageProxy} pageProxy
     */
        static updatePMActType(pageProxy) {
            try {
                let formCellContainer = pageProxy.getControl('FormCellContainer');
                let orderTypeControl = formCellContainer.getControl('TypeLstPkr');
                let pmActControl = formCellContainer.getControl('PMActivityTypeLstPkr');
                let pmActControlValue = libCommon.getListPickerValue(formCellContainer.getControl('PMActivityTypeLstPkr').getValue());
                var pmActCtrlSpecifier = pmActControl.getTargetSpecifier();
                let orderTypePickerValue = libCommon.getListPickerValue(orderTypeControl.getValue());
                pmActCtrlSpecifier.setQueryOptions("$filter=OrderType eq '" + orderTypePickerValue + "'&$orderby=ActivityType");
                return pmActControl.setTargetSpecifier(pmActCtrlSpecifier).then(() => {
                    if (pmActControlValue) {
                        pmActControl.setValue(pmActControlValue);
                    } 
                    return Promise.resolve(true);
                });
            } catch (err) {
