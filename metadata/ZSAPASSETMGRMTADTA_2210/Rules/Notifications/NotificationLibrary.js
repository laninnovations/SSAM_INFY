...
export default class {
        ...
        if (selection.length > 0) {
            ...
            return this.CatalogCodeQuery(context, notif, type).then(function(result) {
                ...
                return targetList.setTargetSpecifier(specifier).then(() => {
                    targetList.setValue(`PMCatalogProfiles(Catalog='${result.Catalog}',CodeGroup='${selection}'`);
                });
            });
        } else {
            targetList.setValue('');
    ...
    static CatalogCodeQuery(context, notification, type) {
        ...
        return Promise.all(reads).then(function(readResults) {
            ...
            return Promise.all(catalogProfileResults).then(function(codeReadResults) {
                ...
                return { 'Catalog': catalogs[type], 'CatalogProfile': readResults[2].getItem(0).CatalogProfile };
            });
        });
    }
    /**
        ...
        if (binding && binding['@odata.type'] === '#sap_mobile.MyNotificationHeader') {
            ...
            if (binding && binding['@odata.readLink'] && (binding['@odata.type'] === '#sap_mobile.MyNotificationItem' || binding['@odata.type'] === '#sap_mobile.MyNotificationTask' || binding['@odata.type'] === '#sap_mobile.MyNotificationActivity')) {
                ...
                binding = {
                    ...
                    // eslint-disable-next-line brace-style
                    'HeaderEquipment': (function() {try { return context.getPageProxy().getControl('FormCellContainer').getControl('EquipHierarchyExtensionControl').getValue(); } catch (e) { return binding.HeaderEquipment||''; }})(),
                    // eslint-disable-next-line brace-style
                    'HeaderFunctionLocation' : (function() {try { return context.getPageProxy().getControl('FormCellContainer').getControl('FuncLocHierarchyExtensionControl').getValue(); } catch (e) { return binding.HeaderFunctionLocation||''; }})(),
                };
                return this.GroupQuery(context, binding, type);
            }
        }
        ...
        return context.read('/SAPAssetManager/Services/AssetManager.service', notificationItem['@odata.readLink'] + specifier, [], '')
            .then(function(data) {
                ...
                let binding = {
                    ...
                    // eslint-disable-next-line brace-style
                    'HeaderEquipment': (function() {try { return context.getPageProxy().getControl('FormCellContainer').getControl('EquipHierarchyExtensionControl').getValue(); } catch (e) { return notificationItem.HeaderEquipment || ''; }})(),
                    // eslint-disable-next-line brace-style
                    'HeaderFunctionLocation' : (function() {try { return context.getPageProxy().getControl('FormCellContainer').getControl('FuncLocHierarchyExtensionControl').getValue(); } catch (e) { return notificationItem.HeaderEquipment || ''; }})(),
                };
    ...
    static NotificationCreateMainWorkCenter(context) {
        ...
        return '';
       /** let equi =  libThis.NotificationCreateUpdateEquipmentLstPkrValue(context);
       let floc =  libThis.NotificationCreateUpdateFunctionalLocationLstPkrValue(context);
        if (equi) {
            return libCom.getEntityProperty(context, `MyEquipments(${equi.toString()})`, 'MaintWorkCenter').then(workCenterId => {
                return workCenterId;
            });
        } else {
            return libCom.getEntityProperty(context, `MyFunctionalLocations(${floc.toString()})`, 'MaintWorkCenter').then(workCenterId => {
                return workCenterId;
            });
        }**/
    }
    ...
    static NotificationCreateMainWorkCenterPlant(context) {
        ...
        let floc =  libThis.NotificationCreateUpdateFunctionalLocationLstPkrValue(context);
         if (equi) {
             return libCom.getEntityProperty(context, `MyEquipments(${equi.toString()})`, 'MaintPlant').then(workCenterId => {
                 return workCenterId;
             });
         } else {
             return libCom.getEntityProperty(context, `MyFunctionalLocations(${floc.toString()})`, 'MaintPlant').then(workCenterId => {
                 return workCenterId;
             });
         }**/
    }
                ...
                if (binding.QMCatalog && binding.QMCodeGroup) {
                    var notification = {
                        ...
                        'HeaderEquipment': (function() {
                            try {
                                if (libVal.evalIsEmpty(binding.HeaderEquipment)) {
                                    ...
                                    return binding.HeaderEquipment;
                                }
                                // eslint-disable-next-line brace-style
                            } catch (e) { return ''; }
                        })(),
                        'HeaderFunctionLocation': (function() {
                            try {
                                if (libVal.evalIsEmpty(binding.HeaderFunctionLocation)) {
                                    return '';
                                } else {
                                    return binding.HeaderFunctionLocation;
                                }
                                // eslint-disable-next-line brace-style
                            } catch (e) { return ''; }
                        })(),
                    ...
                    //return sectionProxy.read('/SAPAssetManager/Services/AssetManager.service', 'NotificationTypes', ['CatalogProfile'], `$filter=NotifType eq '${binding.NotificationType}'`).then(catalogProfileArray => {
                        return this.CatalogCodeQuery(sectionProxy, notification, 'CatTypeCoding').then(catalogProfileArray => {
                        if (catalogProfileArray.CatalogProfile) {
                            return sectionProxy.read('/SAPAssetManager/Services/AssetManager.service', `PMCatalogProfiles(CatalogProfile='${catalogProfileArray.CatalogProfile}',Catalog='${binding.QMCatalog}',CodeGroup='${binding.QMCodeGroup}')`, ['CodeGroup', 'Description'], '').then(pmCatalogProfileArray => {
            ...
            if (notificationType) {
                return pageProxy.read('/SAPAssetManager/Services/AssetManager.service', `NotificationTypes('${notificationType}')`, [], '$expand=DetectionCatalog_Nav').then((result) => {
                    if (result && result.getItem(0)) {
                        ...
                        this.CatalogCodeQuery(pageProxy, notification, 'CatTypeCoding').then(function(value) {
