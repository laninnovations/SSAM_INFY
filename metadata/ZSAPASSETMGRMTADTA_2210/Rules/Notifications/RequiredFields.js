...
export default function RequiredFields(context) {
    ...
    let equipControl = context.evaluateTargetPathForAPI('#Control:EquipHierarchyExtensionControl');
    let funcLocControl = context.evaluateTargetPathForAPI('#Control:FuncLocHierarchyExtensionControl');
    if (!funcLocControl.getValue()){
        if (!equipControl.getValue()){
            required.push('FuncLocHierarchyExtensionControl');
            required.push('EquipHierarchyExtensionControl');
        }
    }
